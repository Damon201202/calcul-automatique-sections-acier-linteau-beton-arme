...
function calculPoutre() {
    ...
    td = parseFloat(document.querySelector('.calcul-four_tg').value);
    ...
    // Alerte hauteur utile
let alertHauteur = document.querySelector('.alert-element_d');

if (h <= d) {
    alertHauteur.innerHTML = `⚠️ Hauteur utile invalide : d = ${d} m ≥ h = ${h} m. La hauteur utile doit être strictement inférieure à celle du linteau.`;
    alertHauteur.style.display = "block";
} else {
    alertHauteur.innerHTML = "";
    alertHauteur.style.display = "none";
}
    ...
    // alert nuance d'acier
let select_nuanceAcier = document.querySelector('.calcul-four-select-nuance_acier');
let alertNuance = document.querySelector('.alert-element-classe-nuance_acier');

switch (select_nuanceAcier.value) {
    case 'A':
        alertNuance.innerHTML = "Attention : Pour l’instant, cette version du calcul prend en compte uniquement l’acier B500B";
        alertNuance.style.display = "block";
        break;

    case 'B':
        alertNuance.innerHTML = "";
        alertNuance.style.display = "none";
        break;
}
...
if (cReel < cNom) {
  ...
  alertEnrobage.style.display = "none"; // ✅ Cache l'alerte
    ...
    // Check if nuance acier is A
let alertNuanceAcier = document.querySelector('.alert-nuance_acier');

if (selectNuanceAcier == 'b500a') {
    alertNuanceAcier.innerHTML = "Attention : Cet outil ne prend pas en charge le calcul pour l’acier de nuance A. Veuillez sélectionner une autre nuance d’acier.";
    alertNuanceAcier.style.display = "block";
} else {
    alertNuanceAcier.innerHTML = "";
    alertNuanceAcier.style.display = "none";
}
    ...
    // Calcul alpha et alerte sur-armement
if (mu <= 0.5) {
...
// === MATERIAUX ===
const materiaux = {
  "Blocs de béton creux": { fk: 2.0, gamma: 2.0 },
  "Maçonnerie béton armé": { fk: 17.0, gamma: 1.0 },
  "Blocs de béton pleins": { fk: 10.0, gamma: 2.0 },
  "Briques pleines": { fk: 8.0, gamma: 2.0 },
  "Pierre calcaire maçonnée": { fk: 8.0, gamma: 2.0 },
  "Maçonnerie de moellons": { fk: 1.0, gamma: 2.0 },
  "Briques silico-calcaires": { fk: 12.0, gamma: 3.0 },
  "Béton cellulaire autoclavé": { fk: 1.0, gamma: 2.0 },
  "Béton non armé": { fk: 8.0, gamma: 2.0 }
};

document.querySelector('.calcul-four_ln').addEventListener('input', calculReactionAppui);
document.querySelector('.calcul-four_gu').addEventListener('input', calculReactionAppui);
document.querySelector('.calcul-four_tg').addEventListener('input', calculReactionAppui);

function calculReactionAppui() {
    // Récupère l'élément <select> et l'option sélectionnée
    const selectElement = document.querySelector('.calcul-four-select-materiau');
    const selectedOption = selectElement.options[selectElement.selectedIndex].text;

    // Récupère les valeurs du matériau sélectionné
    const mat = materiaux[selectedOption];

    if (mat) {
        // Calcul de fcdpl
        const fcdpl = mat.fk / mat.gamma;

        // Effectuer les calculs nécessaires
        const R_appui = (pu * ln) / 2; // Définir pu, ln dans le contexte global ou local
        const a_min = (2 * R_appui) / (b * fcdpl * 1000); // Définir b dans le contexte global ou local
        
        // Alerte profondeur d’appui
let alertProfondeur = document.querySelector('.alert-element_profondeur');

if (a_min > tg) {
    alertProfondeur.innerHTML = `⚠️ La profondeur d’appui effective a = ${tg.toFixed(3)} m est insuffisante (a<sub>min</sub> = ${a_min.toFixed(3)} m).`;
    alertProfondeur.style.display = "block";
} else {
    alertProfondeur.innerHTML = ""; // pas d'alerte si c'est bon
    alertProfondeur.style.display = "none";
}


        // Met à jour les valeurs affichées
        document.querySelector('.calcul-four_reaction_r').innerHTML = R_appui.toFixed(2);
        document.querySelector('.calcul-four-profondeur_a_min').innerHTML = a_min.toFixed(3);
        document.querySelector('#fcdPl-info').innerHTML =
            `Valeur sécuritaire minimale prise en compte : f<sub>cd,pl</sub> = ${mat.fk} / ${mat.gamma} = <strong>${fcdpl.toFixed(2)} MPa</strong>`;
    } else {
        console.error("Matériau non défini ou sélection invalide.");
    }
}
...
if (sigmaCqp <= fck45) {
    ...
    document.querySelector('.alert-element_sigmacqp').innerHTML = `⚠️ Attention : σ<sub>c,qp,ser</sub> = ${sigmaCqp.toFixed(1)} MPa > 0,45·f<sub>ck</sub> = ${fck45.toFixed(1)} MPa.<br>
        La contrainte de compression dépasse la limite admissible : la section doit être redimensionnée.<br>
        Pour y remédier, on peut généralement :
        <br>1. Augmenter la hauteur du linteau (h) <span style="color: red; font-weight: bold;">‼️</span> Ajuster la hauteur utile de la section (d) selon la valeur d<sub>anticipé</sub> proposée.
        <br>2. Utiliser un béton de classe supérieure.`;
